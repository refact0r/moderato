...
class error(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandOnCooldown):
            await utils.utility.error_message(
                ctx,
                f"This command is on cooldown, please retry in `{math.ceil(error.retry_after)} seconds`.",
            )
            return

        if isinstance(error, commands.NoPrivateMessage):
            await utils.utility.error_message(
                ctx, "This command cannot be used in direct messages."
            )
            return
        ...
        if isinstance(error, discord.Forbidden):
            await utils.utility.error_message(
                ctx, "I do not have permission to run this command."
            )
